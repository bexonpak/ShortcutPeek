...
/// Manages the ShortcutPeek icon in the system menu bar (top‑right area,
...
final class MenuBarManager {
  ...
  init(overlayViewModel: OverlayViewModel) {
    ...
    statusItem.button?.image = NSImage(systemSymbolName: "command",
                                       accessibilityDescription: "ShortcutPeek")
    statusItem.button?.image?.isTemplate = true

    // Dim the icon when ShortcutPeek is disabled.
  ...
  private func buildMenu() -> NSMenu {
    ...
    let toggleItem = NSMenuItem(
      title: overlayViewModel.isEnabled ? "Disable ShortcutPeek" : "Enable ShortcutPeek",
