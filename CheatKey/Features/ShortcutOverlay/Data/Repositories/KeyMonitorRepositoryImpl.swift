...
/// monitor (events when CheatKey itself is frontmost) so the overlay
/// works regardless of which app has focus.
///
/// ## Accessibility permission
/// The *global* monitor requires Accessibility access to receive events
/// from other processes.  The *local* monitor always works for CheatKey's
/// own events.
@MainActor
final class KeyMonitorRepositoryImpl: KeyMonitorRepository {
  ...
  func start() {
    ...
    // Local monitor: receives events for CheatKey's own windows.
